...
begin
     ...
     End;
     //
     If uFuncoes.Empty(dteDTINIC.Text) Then
     begin
          ShowMessage('Campo obrigatório!');
          dteDTINIC.SetFocus;
          Exit;
     End;
     //
     If uFuncoes.Empty(dteDTFINA.Text) Then
     begin
          ShowMessage('Campo obrigatório!');
          dteDTFINA.SetFocus;
          Exit;
     ...
     //
         ...
         begin
              ...
              txtTexto.Lines.Add(uFuncoes.Replicate(' ',0)+'secretaria  no  período  de   '+ dteDTINIC.Text+ '  a  '+dteDTFINA.Text
                                       +'  do  corrente ano.');
              txtData.Caption     := 'São Luis - MA,  '+Copy(DatetoStr(Date),1,2)+
              ...
              txtTexto2.Lines.Add(uFuncoes.Replicate(' ',0)+'secretaria  no  período  de   '+ dteDTINIC.Text+ '  a  '+dteDTFINA.Text
                                       +'  do  corrente ano.');
              txtData2.Caption     := 'São Luis - MA,  '+Copy(DatetoStr(Date),1,2)+
                                     '   de   '+ uFuncoes.MesExtenso(Date())+'  de  '+Copy(DatetoStr(Date),7,4)+'.';
              //
              qrRelatorio.Preview;
              Close;
